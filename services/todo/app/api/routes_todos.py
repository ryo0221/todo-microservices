...
def list_my_todos(db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
...
def update_todo(todo_id: int, payload: TodoUpdate, db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
