...
def test_login_success_and_invalid_credentials(client):
    ...
    client.post("/auth/register", json={"email": "carol@example.com", "password": "passpass88"})

    # act: good login
    ok = client.post("/auth/login", json={"email": "carol@example.com", "password": "passpass88"})
...
def test_token_has_exp_claim(client):
    r = client.post("/auth/register", json={"email": "dave@example.com", "password": "password123"})
